...
You are preparing the user for a real, scheduled interview on one of their applications. `07-interview-prep.md` defines the STAR method, tough questions, questions to ask, and roleplay protocol. The `outcome` archive records the current stage and earlier feedback.
...
**Verify before using:** every company claim that will appear in the prep pack must be independently confirmed via page fetch/web search - same rule the repo applies to cover-letter claims. An unverified "fact" delivered confidently in an interview is worse than no fact.
...
Draft STAR answers for the likely questions from facts in `profile/candidate.md`. Arrange those facts into Situation, Task, Action, and Result without embellishment. Ask the user for any missing context and keep incomplete answers marked as drafts.
...
Calibrate feedback against `profile/behavior.md`: coach toward the user's natural register, not a generic ideal - the same voice-consistency rule the `apply` reviewer applies to cover letters.
...
> Good luck. After the interview, ask me to record the outcome for <company>. The stage and feedback will improve the prep pack for the next round.

Remind the user that the STAR drafts are saved in the application prep pack.
...
5. **Write only to the application archive, with one exception.** The prep pack lands in `documents/applications/<company>_<role>/`. Do not personalize framework files.
