...
import { boardsFor, type RemoteJobCard, type RemoteJobDetail } from "./types.ts"
...
export async function runRemoteCli(argv: readonly string[]): Promise<void> {
  try {
    ...
    if (command === "search") {
      ...
      if (result.failures.length === boardsFor().length) {
        ...
        throw new Error(`Every remote board failed (${details})`)
      }
    ...
    if (command === "detail") {
      ...
      if (args.positionals.length !== 2) throw new Error("detail requires one remote job ID or URL")
...
function renderSearch(results: readonly RemoteJobCard[], total: number, page: number, format: OutputFormat): string {
...
function renderDetail(job: RemoteJobDetail, format: OutputFormat): string {
...
function helpText(): string {
  ...
  return `Usage: bun run .agents/skills/remote-search/cli/src/cli.ts <command> [flags]\n\n` +
    `Source: remote boards (${boards})\n\n` +
    "Commands:\n" +
    "  search  Search public remote-board postings\n" +
