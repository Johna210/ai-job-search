...
3. Read `profile/search.md` from the repository root for the search strategy
...
2. Translate the query terms from `profile/search.md` into that portal's flag format (e.g. `--key`, `--search-string`, `--query`, filter codes — whatever the portal's SKILL.md specifies).
...
If the user picks a number, invoke the **apply** skill workflow. Evaluate fit first, then draft the CV and cover letter only after approval.
