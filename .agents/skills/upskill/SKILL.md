...
3. Read `profile/candidate.md` to get the candidate's current skills and experience.
...
### Targeted mode
1. Use page fetch to retrieve the job posting from the URL.
...
### Aggregate mode
For each job row in the tracker, you do not have the full posting — use the `role`, `sector`, and `notes` columns to infer likely required skills. If the row has a `source` URL, you may optionally page fetch it for more detail, but skip if the URL is missing or dead.
