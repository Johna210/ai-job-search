...
if (search.code !== 0) throw new Error(`remote search failed: ${search.stderr.trim()}`)

const result = parseJson(search.stdout)
const results = result.results
if (!Array.isArray(results) || results.length === 0 || !isObject(results[0])) {
  throw new Error("Remote live search returned no result")
}

const id = requiredText(results[0], "id")
const title = requiredText(results[0], "title")
const url = requiredText(results[0], "url")
if (!/^https?:\/\//i.test(url)) throw new Error("Remote result returned a non-URL posting link")

const detail = await runCli(["detail", id, "--format", "json"])
if (detail.code !== 0) throw new Error(`remote detail failed: ${detail.stderr.trim()}`)
...
if (/<\/?[a-z][^>]*>/i.test(description) || description.includes("&amp;")) {
  throw new Error("Remote detail description still contains markup or entities")
}

process.stdout.write(`remote: ${title} -> detail ${id}\n`)
...
function parseJson(output: string): Record<string, unknown> {
  ...
  if (!isObject(value)) throw new Error("Remote CLI did not return a JSON object")
  return value
}

function requiredText(record: Record<string, unknown>, field: string): string {
  const value = textValue(record[field])
  if (!value) throw new Error(`Remote result has no ${field}`)
  return value
}
