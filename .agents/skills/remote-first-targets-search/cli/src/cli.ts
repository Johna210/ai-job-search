#!/usr/bin/env bun
// Self-contained CLI for searching live job listings at remote-first companies.
// Reads each company's public careers API (Greenhouse, Lever, Ashby, or
// SmartRecruiters) — no authentication, no API keys, no dependencies beyond
// `bun`.
//
// Remote-first status comes from a curated list. A posting's actual location
// rules live in the posting: always check it before applying.
...
const COMPANIES: Company[] = [
  { key: "gitlab", name: "GitLab", ats: "greenhouse", token: "gitlab", country: "USA" },
  { key: "octopusdeploy", name: "Octopus Deploy", ats: "greenhouse", token: "octopusdeploy", country: "Australia" },
  { key: "huntress", name: "Huntress", ats: "greenhouse", token: "huntress", country: "USA" },
  { key: "buffer", name: "Buffer", ats: "ashby", token: "buffer", country: "USA" },
  { key: "testgorilla", name: "TestGorilla", ats: "ashby", token: "testgorilla", country: "Netherlands" },
  { key: "socket", name: "Socket", ats: "ashby", token: "socket", country: "USA" },
  { key: "kinsta", name: "Kinsta", ats: "lever", token: "kinsta", country: "Hungary" },
  { key: "doist", name: "Doist", ats: "smartrecruiters", token: "doist", country: "USA" },
  { key: "automattic", name: "Automattic", ats: "smartrecruiters", token: "automattic", country: "USA" },
  { key: "duckduckgo", name: "DuckDuckGo", ats: "smartrecruiters", token: "duckduckgo", country: "USA" },
]
...
const HELP = `remote-first-targets-cli — search live jobs at remote-first companies
...
EXAMPLES
  bun run src/cli.ts search -q "backend"
  bun run src/cli.ts search -c gitlab -q "python" --jobage 14 --format table
  bun run src/cli.ts search -q "engineer" --remote -n 20
  bun run src/cli.ts companies
  bun run src/cli.ts detail -c gitlab 1234567

Remote-first status is per company. Each posting states its own location rules;
