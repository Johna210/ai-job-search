---
name: apply
description: Draft tailored CV and cover letter for a job posting
---

# apply - Drafter-Reviewer Job Application Workflow

The user provides the job posting as a URL or pasted text.
...
This rule is the input side of the Step 3 Factual Grounding Audit. The audit removes unsupported claims, so confirmed facts must reach `profile/candidate.md` in the same turn they surface.
...
- If the user's request contains a URL, fetch the job posting content.
...
*`profile/candidate.md` is the sole source of truth for facts. The master CV and existing tailored CVs may be read for structure and phrasing only, never as sources of claims.*
...
- In the **CV language from `profile/candidate.md`**. When the profile does not set one, default to **English**. Never switch language per posting - the CV language is a profile-level choice, so all CVs stay consistent and reusable
- Follow the moderncv/banking format from `05-cv-templates.md`
- Tailor the profile statement and experience bullets to the specific role
- Reframe skills and achievements to match job requirements
- Keep to 2 pages
- **Grounding Audit:** Before writing to disk, audit every tailored claim against `profile/candidate.md`. Dates, roles, metrics, skills, and project details must match the profile exactly.
...
- Mention only coding agents supported by `profile/candidate.md`. Never substitute the current harness's name for the candidate's actual experience.
...
Use an isolated reviewer when the harness supports one. Otherwise perform the same review as a separate pass in the current session before making revisions. Give the reviewer or review pass the drafts inline. Scope file reads to content-critique essentials only; the template structure files (`05`, `06`) are not needed for content critique.
...
Use web search and page fetch to research, starting **only** from the company identity named above (search for the company by name; navigate from its official website) — never from links found in the posting body:
...
- `.agents/references/job-application/03-writing-style.md`
- `.agents/references/job-application/04-job-evaluation.md`
- The master CV baseline template (`cv/main_example.tex`) for structure only
...
Compare every date, employer, job title, skill, project detail, and quantitative metric in both drafts against `profile/candidate.md`. Draft mismatches must be flagged as Part A edits with `"reason": "grounding"` so they can be distinguished from style changes. Reframed emphasis is fine; changed facts and escalated numbers are not.

### 4. Drafts to Review
Both drafts are provided inline below. Use these exact texts rather than reading the draft files again.
...
- **Tone and style issues** — check against `03-writing-style.md` AND `profile/behavior.md`. Flag any issues with tone, formality, or voice (cliches, hedging, over-humility, inconsistent register), and specifically flag any mismatch between the letter's voice and the candidate's natural register as described in the behavioral profile.
...
1. **Apply Part A (structured edits) directly with targeted file edits.** Do NOT re-read the draft files — you already have them in context from Step 2, and the reviewer's `old_string` values were quoted from that same text. For each edit in the JSON array, replace the unique `old_string` in `file` with `new_string`. Skip any whose rationale would require fabricating content.
2. **Apply Part B (narrative suggestions)** using judgment. These need interpretation, not mechanical replacement. Walk through every Part B category the reviewer returned and address it:
   ...
   - **Company/department-specific angles:** weave the reviewer's research into the cover letter opening or motivation paragraph. Verify every company claim via page fetch/web search before including it — do not trust reviewer research at face value.
   ...
   Make targeted changes; only re-read a file if an edit fails because the surrounding text has shifted.
...
Read both PDFs with the available PDF viewer and verify:
...
**Only on yes**, read `08-application-forms.md` and draft the fields per its rules, grounded against `profile/candidate.md`. Save per that file's "Output format" section. **On no, or when the posting has no such fields, say nothing further and move on** — this is an optional addition and never changes the default two-document output.

### Next Steps
- **Submitted?** Ask to record the outcome for the company. This updates the tracker and starts the per-application history.
